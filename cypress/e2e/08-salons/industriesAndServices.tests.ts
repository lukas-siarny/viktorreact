	...
	context('Industries and services', () => {
		...
		it('Update service settings', () => {
			...
			cy.wait('@getSalonServices').then((interceptorGetSalonServices: any) => {
				...
				cy.get('main.ant-layout-content').then(($body) => {
					if ($body.find(`.ant-collapse-item-active`).length && !$body.find(`.ant-collapse-item-active:first .${CYPRESS_CLASS_NAMES.ASSIGN_SERVICES_BUTTON}`)) {
						cy.get('.ant-table:first .ant-table-row:first')
							.as('firstRow')
								...
								cy.wait(['@getSalonService', '@getCategory', '@getCategories', '@getEmployees']).then(
									([interceptorGetSalonSerivce, interceptorGetCategory, interceptorGetCategories, interceptorGetEmlpoyees]: any[]) => {
										...
										if (actions.includes(CRUD_OPERATIONS.ALL) || actions.includes(CRUD_OPERATIONS.UPDATE)) {
											...
											if ($body.find(`#${FORM.SERVICE_FORM}-descriptionLocalizations-use`).length) {
												cy.get(`#${FORM.SERVICE_FORM}-descriptionLocalizations-use > button`).then(($switchButton) => {
													if (!$switchButton.hasClass('ant-switch-checked')) {
														cy.wrap($switchButton).click()
													}
													cy.setInputValue(
														FORM.SERVICE_FORM,
														'descriptionLocalizations-defualtLanguage',
														service.descriptionLocalizations.default,
														true,
														true
													)
													cy.setInputValue(FORM.SERVICE_FORM, 'descriptionLocalizations-enLanguage', service.descriptionLocalizations.en, true, true)
												})
											}

											// add employee from Select with async search (at least one employee must exist in the salon in order to test work properly!!)
											...
											cy.wait('@getEmployees').then((interceptorGetEmlpoyeesSearch: any) => {
												...
												cy.wait('@updateSalonService').then((interceptorUpdateSalonService: any) => {
													...
													cy.get(`#${FORM.SERVICE_FORM}-settings-enabledB2cReservations > button`).then(($switchButton) => {
														if (!$switchButton.hasClass('ant-switch-checked')) {
															cy.wrap($switchButton).click()
														} else {
															cy.get(`#${FORM.SERVICE_FORM}-settings-autoApproveReservations > button`).should('be.disabled')
														}
														cy.clickButton('settings-autoApproveReservations', FORM.SERVICE_FORM, true)
														cy.wait('@updateSalonService').then((interceptorUpdateSalonService2: any) => {
															expect(interceptorUpdateSalonService2.response.statusCode).to.equal(200)
														})
													})
												})
											})
										...
										if (actions.includes(CRUD_OPERATIONS.ALL) || actions.includes(CRUD_OPERATIONS.UPDATE)) {
											cy.get('#service-employees-list')
												...
												.then((btnID) => {
													...
													cy.wait(1000)
													cy.setInputValue(
														FORM.EMPLOYEE_SERVICE_EDIT,
														'employeePriceAndDurationData-durationFrom',
														service.employeeService.durationFrom,
														true,
														true
													)
													cy.setInputValue(
														FORM.EMPLOYEE_SERVICE_EDIT,
														'employeePriceAndDurationData-priceFrom',
														service.employeeService.priceFrom,
														true,
														true
													)
		...
		it('Services settings', () => {
			...
			if (actions.includes(CRUD_OPERATIONS.ALL) || actions.includes(CRUD_OPERATIONS.READ) || actions.includes(CRUD_OPERATIONS.UPDATE)) {
				cy.wait('@getSalonServices').then((interceptionServices: any) => {
					expect(interceptionServices.response.statusCode).to.equal(200)
					// change RS settings
					if (actions.includes(CRUD_OPERATIONS.ALL) || actions.includes(CRUD_OPERATIONS.UPDATE)) {
						cy.clickButton(ENABLE_RS_BUTTON_ID, undefined, true)
						cy.wait('@updateReservationsSettings').then((interceptionRsSettings: any) => {
							expect(interceptionRsSettings.response.statusCode).to.equal(200)
							cy.checkSuccessToastMessage()
						})
					} else {
						cy.clickButton(ENABLE_RS_BUTTON_FAKE_BUTTON_ID)
						cy.checkForbiddenModal()
					}
