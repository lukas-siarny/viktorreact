	...
	it('Admin dashboard', () => {
		...
		if (actions.includes(CRUD_OPERATIONS.ALL) || actions.includes(CRUD_OPERATIONS.READ)) {
			if (!salonID && !(role in SALON_ROLES)) {
				...
				cy.wait(['@getSalonTimeStats', '@getReservationTimeStats', '@getNotinoDashboard']).then(([interceptionSalonTimeStats, interceptionReservationTimeStats, interceptionNotinoDashboard]: any[]) => {
